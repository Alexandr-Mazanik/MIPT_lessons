...
SIDEWALK_COLOR = (83, 108, 103)
SKY_COLOR = (183, 196, 200)
CLOUD_COLOR = (133, 147, 144, 120)
HOUSE_COLORS = [
    (111, 145, 138),
    (183, 200, 196),
    (147, 169, 170),
...
def draw_scene(screen):
    ...
    number_of_houses = 3
    car_width = SCREEN_WIDTH // 2
    car_height = SCREEN_HEIGHT // 7
    car_position = (SCREEN_WIDTH // 3, 17 * SCREEN_HEIGHT // 20)

    draw_houses(screen, number_of_houses, sky_height)

    draw_the_road(screen)

    car, smoke = make_a_car(car_width, car_height)
    draw_a_car(screen, car, smoke, car_position)
...
def draw_the_road(screen):
    ellipse(screen, ROAD_COLOR,
            (0, 3 * SCREEN_HEIGHT // 4,
             4 * SCREEN_WIDTH, SCREEN_HEIGHT))


def draw_a_car(screen, car, smoke, position):
    x, y = position
